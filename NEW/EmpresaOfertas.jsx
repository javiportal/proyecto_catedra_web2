...
export default function EmpresaOfertas() {
  ...
  useEffect(() => { if (empresaId) { fetchOfertas(); fetchRubros() } }, [empresaId])
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editing) {
        delete payload.estado // don't reset estado on edit
      ...
      setForm(emptyForm); setEditing(null); setShowForm(false); fetchOfertas()
  ...
  const handleDelete = async (id) => {
    ...
    else { toast.success('Oferta eliminada'); fetchOfertas() }
  }
    ...
    <div className="admin-page">
      ...
      {showForm && (
        <div className="admin-form-card">
          ...
          <F label="Título" name="titulo" />
          <F label="Descripción" name="descripcion" type="textarea" />
          ...
          <div className="form-grid-2">
            <F label="Fecha Límite Cupón" name="fecha_limite_cupon" type="date" />
            <F label="Rubro" name="rubro_id" type="select" />
          </div>
          <div className="form-grid-2">
            <F label="Cantidad Límite (vacío = ilimitado)" name="cantidad_limite" type="number" min="1" />
      ...
      <div className="space-y-4">
        {ofertas.map(o => (
          <div key={o.id} className="admin-offer-card">
            <div className="flex justify-between items-start mb-2">
              <div>
                <h3 className="text-lg font-bold text-[var(--text)]">{o.titulo}</h3>
                <p className="text-sm text-[var(--text-muted)]">{o.rubros?.nombre}</p>
              </div>
              <span className={`admin-status-badge ${(o.estado || '').toLowerCase()}`}>{o.estado}</span>
            </div>
            <p className="text-sm text-[var(--text-muted)] mb-2">{o.descripcion}</p>
            <div className="flex gap-3 text-sm text-[var(--text-muted)] mb-3">
              <span>Regular: ${o.precio_regular}</span>
              <span>Oferta: <strong className="text-[var(--gold)]">${o.precio_oferta}</strong></span>
        ...
        {ofertas.length === 0 && <div className="text-center py-12 text-[var(--text-muted)]">No has creado ofertas aún.</div>}
      </div>
    </div>
  )
}
