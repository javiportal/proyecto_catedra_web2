  ...
  return (
    <div className="admin-page">
      <div className="admin-header">
        <h1 className="admin-title">Aprobacion de Ofertas</h1>
      ...
      <div className="space-y-4">
        {filtradas.map(oferta => (
          <div key={oferta.id} className="admin-offer-card">
            <div className="flex justify-between items-start mb-3">
              <div>
                <h3 className="text-lg font-bold text-main">{oferta.titulo}</h3>
                <p className="text-sm text-gold">{oferta.empresas?.nombre} ({oferta.empresas?.codigo})</p>
              </div>
              <span className={`admin-status-badge ${(oferta.estado || '').toLowerCase()}`}>
                {oferta.estado}
              </span>
            </div>
            <p className="text-sm text-muted mb-3">{oferta.descripcion}</p>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm mb-4">
              <div><span className="text-muted">Rubro:</span> <span className="text-main">{oferta.rubros?.nombre}</span></div>
              <div><span className="text-muted">Regular:</span> <span className="text-main">${oferta.precio_regular}</span></div>
              <div><span className="text-muted">Oferta:</span> <span className="text-gold">${oferta.precio_oferta}</span></div>
              <div><span className="text-muted">Vigencia:</span> <span className="text-main">{oferta.fecha_inicio} - {oferta.fecha_fin}</span></div>
            </div>
        ...
        {filtradas.length === 0 && (
          <div className="text-center py-12 text-muted">No hay ofertas con este filtro.</div>
        )}
      </div>
    </div>
  )
